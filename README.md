...
## 개요
사용자에게 편의점의 남아 있는 물건을 출력하고, 다양한 혜택들에 대한 출려과
결과에 대한 영수증을 발행하는 프로그램
...
3. [x] 구매할 상품을 입력받는 기능\
   3-1. [x] ,로 단어를 구분하고 []안에 있는지 판단\
   3-2. [x] -로 물건과 수량 구분\
   3-3. [x] 물건이 실제로 존재하는지 확인하는 기능\
   3-4. [x] 주문한 물건의 수량이 존재하는 수량보다 크지 않은지 확인하는 기능
   3-5. [] 프로모션 상품부터 사용되게끔 만드는 기능
4. [] 프로모션에 대한 정보를 가져오고, 현재날짜와 비교하여 프로모션의 적용을 판단하는 기능
5. [] 구매할 상품이 프로모션 적용상품인지 판단하는 기능
6. [] 구매할 상품의 재고가 모자르지 않는지 판단하는 기능
7. [] 구매한 것들에 대한 계산을 하는 기능
8. [] 영수증 발행하는 기능

## 예외처리
1. [] "[" 로 시작했는데 "]" 로 닫히지 않은 경우
2. [] 중간에 "[]"가 들어온 경우
3. [] 개수를 -를 기준으로 자르기
